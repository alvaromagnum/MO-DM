export default class Drawflow {
    ...
    click(e) {
        ...
        if (this.editor_mode === 'fixed') {
            ...
            if (e.target.id === "projectConfigCanvas") this.editor_selected = true; // TODO MAGNUM
            ...
            if (ele_last.classList[0] === 'input' || (this.force_first_input && (ele_last.closest(".drawflow_content_node") != null || ele_last.classList[0] === 'drawflow-node'))) {
                ...
                if (output_id !== input_id && input_class !== false) {
                    ...
                    if (this.container.querySelectorAll('.connection.node_in_' + input_id + '.node_out_' + output_id + '.' + output_class + '.' + input_class).length === 0) {
                        ...
                        // TODO - MAGNUM
                        if (this.drawflow.drawflow[this.module].data[id_output].name === "step") {
                            if (this.drawflow.drawflow[this.module].data[id_input].name === "step") {
                                if (output_class !== "output_1") return;
                            } else if (this.drawflow.drawflow[this.module].data[id_input].name === "decision") {
                                if (output_class !== "output_2") return;
                            } else return;
                        } else if (this.drawflow.drawflow[this.module].data[id_output].name === "decision") {
                            if (this.drawflow.drawflow[this.module].data[id_input].name !== "stakeholder") {
                                return;
                            }
                        }
    ...
    addNode(name, num_in, num_out, ele_pos_x, ele_pos_y, classoverride, data, html, typenode = false) {
        ...
        for (var x = 0; x < num_in; x++) {
            ...
            if (name === "stakeholder" && x === 0) {
                input.classList.add("stakeholder-input");
            }
        ...
        for (var x = 0; x < num_out; x++) {
            ...
            // TODO - MAGNUM
            if (name === "step" && x === 0) {
                output.classList.add("step-output-1");
            }

            if (name === "step" && x === 1) {
                output.classList.add("step-output-2");
            }
    ...
    addNodeImport(dataNode, precanvas) {
        ...
        Object.keys(dataNode.inputs).map(function (input_item, index) {
            ...
            // TODO MAGNUM
            if (dataNode.name === "step" && index === 0) {
                input.classList.add("step-input");
            }

            if (dataNode.name === "stakeholder" && index === 0) {
                input.classList.add("stakeholder-input");
            }
        ...
        for (var x = 0; x < Object.keys(dataNode.outputs).length; x++) {
            ...
            // TODO MAGNUM
            if (dataNode.name === "step" && x === 0) {
                output.classList.add("step-output-1");
            }

            if (dataNode.name === "step" && x === 1) {
                output.classList.add("step-output-2");
            }
            ...
            this.drawflow.drawflow[moduleName].data[itemx.node].outputs[itemx.input].connections.forEach((itemz, g) => {
                if (itemz.node == id) {
                    ...
                    if (parseInt(input_class_id) < parseInt(output_id)) {
                        ...
                        if (itemz.points) {
                            this.drawflow.drawflow[moduleName].data[itemx.node].outputs[itemx.input].connections[g] = {
                                node: itemz.node,
            ...
            this.drawflow.drawflow[moduleName].data[itemx.node].inputs[itemx.output].connections.forEach((itemz, g) => {
                if (itemz.node == id) {
                    ...
                    if (parseInt(output_class_id) < parseInt(input_id)) {
                        ...
                        if (itemz.points) {
                            this.drawflow.drawflow[moduleName].data[itemx.node].inputs[itemx.output].connections[g] = {
                                node: itemz.node,
